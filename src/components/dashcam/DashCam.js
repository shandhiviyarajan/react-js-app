...
class DashCam extends Component {
    ...
    constructor(props, context) {
      ...
      onKeyPress = (e) => {
        if(e.which === 13) {
            if(e.target.value){
                axios.get(BASE_URL_ADMIN+"/dash-cam/list/?search="+e.target.value)
                .then(res => {
                    this.setState({ cam_id: res.data.data.dashCams[0].cam_id })
                    this.setState({ position_id: res.data.data.dashCams[0].position_id })
                    this.setState({ status: res.data.data.dashCams[0].status })
                    this.setState({ created_at: res.data.data.dashCams[0].created_at })
      ...
      onPaginationClick= ()=> {
        ...
        axios.get(BASE_URL_ADMIN+"/dash-cam/list?page="+this.state.page)
        .then(res => {
            {this.setState({ dashcamList: res.data.data.dashCams })}
      ...
      onPaginationClickFirst= ()=> {
        axios.get(BASE_URL_ADMIN+"/dash-cam/list")
        .then(res => {
            {this.setState({ dashcamList: res.data.data.dashCams })}
            {localStorage.setItem('dashcampage', res.data.data.total)}
          });
    ...
    componentDidMount() {  
      ...
      onClickSearch = (id) =>{
        if(id){
            axios.get(BASE_URL_ADMIN+"/dash-cam/list/?search="+id)
            .then(res => {
                {this.setState({ cam_id: res.data.data.dashCams[0].cam_id })}
                {this.setState({ position_id: res.data.data.dashCams[0].position_id })}
                {this.setState({ status: res.data.data.dashCams[0].status })}
        ...
        <div className="main-content-container container-fluid px-4">
          ...
          <div className="page-header row no-gutters py-4">
            <div className="col-12 col-sm-6 text-center text-sm-left mb-0">
              <h3 className="page-title">Dash Cam</h3>
            </div>
            ...
            <div className="col">
              <div className="card card-small mb-4">
                ...
                <div className="form-row">
                    ...
                    <input type="text" className="form-control" id="searchValue" onKeyPress={this.onKeyPress}name="searchValue" placeholder="Search by Dash-cam ID" value={this.state.searchValue} onChange={this.onChange} 
                    />
                    </div>
                    <div className="form-group col-md-4">
                ...
                <div className="card-body p-0 pb-3 text-center">
                  ...
                  <table className="table mb-0">
                    <thead className="bg-light">
                      <tr>
                          <th scope="col">Dash-cam ID</th>
                          <th scope="col">Position ID</th>
                          <th scope="col">Status</th>
                      </tr>
                    </thead>
                    {this.state.dashcamList.map(row => {
                        return(
                            <tr>
                            <td key={row.cam_id}>{row.cam_id}</td>
                            <td key={row.position_id}>{row.position_id}</td>
          ...
          <Modal.Body>
                  ...
                  <div class="card-body p-0">
                    <ul class="list-group list-group-small list-group-flush">
                      ...
                      <label htmlFor>Dash-cam ID : </label>
                        <span class="ml-auto text-right text-semibold text-reagent-white">{this.state.cam_id}</span>
                      </li>
                      <li class="list-group-item d-flex px-3">
